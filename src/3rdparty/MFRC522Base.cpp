 ...
 */
void MFRC522Base::PCD_SetRegisterBitMask(	PCD_Register reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
void MFRC522Base::PCD_ClearRegisterBitMask(	PCD_Register reg,	///< The register to update. One of the PCD_Register enums.
										uint8_t mask			///< The bits to clear.
									  ) {
	uint8_t tmp;
	tmp = PCD_ReadRegister(reg);
	PCD_WriteRegister(reg, tmp & (uint8_t)(~mask));		// clear bit mask
 ...
 */
MFRC522Base::StatusCode MFRC522Base::PCD_CalculateCRC(	uint8_t *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
 ...
 */
void MFRC522Base::PCD_AntennaOn() {
 ...
 */
void MFRC522Base::PCD_AntennaOff() {
 ...
 */
uint8_t MFRC522Base::PCD_GetAntennaGain() {
 ...
 */
void MFRC522Base::PCD_SetAntennaGain(uint8_t mask) {
 ...
 */
bool MFRC522Base::PCD_PerformSelfTest() {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::PCD_TransceiveData(	uint8_t *sendData,		///< Pointer to the data to transfer to the FIFO.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::PCD_CommunicateWithPICC(	uint8_t command,		///< The command to execute. One of the PCD_Command enums.
	...
	if (backData && backLen && checkCRC) {
		...
		MFRC522Base::StatusCode status = PCD_CalculateCRC(&backData[0], *backLen - 2, &controlBuffer[0]);
		if (status != STATUS_OK) {
			return status;
		}
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::PICC_RequestA(	uint8_t *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::PICC_WakeupA(	uint8_t *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
											uint8_t *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
										) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
MFRC522Base::StatusCode MFRC522Base::PICC_REQA_or_WUPA(	uint8_t command, 		///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
												uint8_t *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
												uint8_t *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
											) {
	uint8_t validBits;
	MFRC522Base::StatusCode status;
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::PICC_Select(	Uid *uidValue,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
	...
	while (!uidComplete) {
		...
		switch (cascadeLevel) {
			case 1:
				...
				uidIndex = 0;
				useCascadeTag = validBits && uidValue->size > 4;	// When we know that the UID has more than 4 bytes
				break;
			
			case 2:
				buffer[0] = PICC_CMD_SEL_CL2;
				uidIndex = 3;
				useCascadeTag = validBits && uidValue->size > 7;	// When we know that the UID has more than 7 bytes
				break;
		...
		if (bytesToCopy) {
			...
			for (count = 0; count < bytesToCopy; count++) {
				buffer[index++] = uidValue->uidByte[uidIndex + count];
			}
		}
		...
		for (count = 0; count < bytesToCopy; count++) {
			uidValue->uidByte[uidIndex + count] = buffer[index++];
		}
		...
		if (responseBuffer[0] & 0x04) { // Cascade bit set - UID not complete yes
			...
			uidComplete = true;
			uidValue->sak = responseBuffer[0];
		}
	} // End of while (!uidComplete)
	
	// Set correct uid->size
	uidValue->size = 3 * cascadeLevel + 1;
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
MFRC522Base::StatusCode MFRC522Base::PICC_HaltA() {
	MFRC522Base::StatusCode result;
	uint8_t buffer[4];
 ...
 */
MFRC522Base::StatusCode MFRC522Base::PCD_Authenticate(uint8_t command,		///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
											uint8_t blockAddr, 	///< The block number. See numbering in the comments in the .h file.
											MIFARE_Key *key,	///< Pointer to the Crypteo1 key to use (6 bytes)
											Uid *uidValue			///< Pointer to Uid struct. The first 4 bytes of the UID is used.
											) {
	...
	for (uint8_t i = 0; i < 4; i++) {				// The last 4 bytes of the UID
		sendData[8+i] = uidValue->uidByte[i+uidValue->size-4];
	}
 ...
 */
void MFRC522Base::PCD_StopCrypto1() {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_Read(	uint8_t blockAddr, 	///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_Write(	uint8_t blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_Ultralight_Write(	uint8_t page, 		///< The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_Decrement(	uint8_t blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_Increment(	uint8_t blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_Restore(	uint8_t blockAddr ///< The block (0-0xff) number.
											) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_TwoStepHelper(	uint8_t command,	///< The command to use
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_Transfer(	uint8_t blockAddr ///< The block (0-0xff) number.
											) {
	MFRC522Base::StatusCode result;
	uint8_t cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
  */
MFRC522Base::StatusCode MFRC522Base::MIFARE_GetValue(uint8_t blockAddr, int32_t *value) {
	MFRC522Base::StatusCode status;
	uint8_t buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::MIFARE_SetValue(uint8_t blockAddr, int32_t value) {
	uint8_t buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::PCD_NTAG216_AUTH(uint8_t* passWord, uint8_t pACK[]) //Authenticate with 32bit password
{
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Base::StatusCode MFRC522Base::PCD_MIFARE_Transceive(	uint8_t *sendData,		///< Pointer to the data to transfer to the FIFO. Do NOT include the CRC_A.
 ...
 */
const char *MFRC522Base::GetStatusCodeName(MFRC522Base::StatusCode code	///< One of the StatusCode enums.
										) {
	switch (code) {
 ...
 * @return PICC_Type
 */
MFRC522Base::PICC_Type MFRC522Base::PICC_GetType(uint8_t sak		///< The SAK byte returned from PICC_Select().
										) {
 ...
 * @return const __FlashStringHelper *
 */
const char *MFRC522Base::PICC_GetTypeName(PICC_Type piccType	///< One of the PICC_Type enums.
													) {
	switch (piccType) {
 ...
 * @return bool
 */
bool MFRC522Base::PICC_IsNewCardPresent() {
	...
	MFRC522Base::StatusCode result = PICC_RequestA(bufferATQA, &bufferSize);
 ...
 * @return bool
 */
bool MFRC522Base::PICC_ReadCardSerial() {
	MFRC522Base::StatusCode result = PICC_Select(&uid);
